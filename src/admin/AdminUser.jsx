...
export default function AdminUser() {
  ...
  const visibleLogs = useMemo(() => logs.filter((log) => {
    const text = `${log.action || ""} ${log.actor_role || ""} ${log.actor_email || ""} ${log.actor_name || ""} ${log.target_email || ""} ${log.target_name || ""} ${log.actor_user_id || ""} ${log.target_user_id || ""}`.toLowerCase();
    ...
    <section className="admin-panel-card admin-users-panel">
      ...
      {tab === "administrators" ? (
        ...
        <div className="admin-audit-section">
          ...
          {logsLoading ? <p>Loading audit logs...</p> : <div className="admin-audit-table"><div className="admin-audit-row admin-users-table-head"><span>Time</span><span>Admin</span><span>Action</span><span>Module</span><span>Target</span><span>Result</span></div>{visibleLogs.map((log) => { const result = log.metadata?.result || "success"; return <button className="admin-audit-row" key={log.id} type="button" onClick={() => setSelected(log)}><span>{formatDate(log.created_at)}</span><span title={log.actor_user_id || undefined}>{log.actor_email || log.actor_name || log.actor_user_id || "System"}</span><span>{log.action}</span><span>{log.metadata?.module || "—"}</span><span title={log.target_user_id || undefined}>{log.target_email || log.target_name || log.target_user_id || "—"}</span><span className={result === "failed" ? "audit-failed" : "audit-success"}>{result}</span></button>; })}{!visibleLogs.length && <p>No audit entries found.</p>}</div>}
        </div>
      )}

      {selected && <div className="admin-details-overlay" onMouseDown={(e) => e.target === e.currentTarget && setSelected(null)}><aside className="admin-details-drawer"><button className="admin-details-close" onClick={() => setSelected(null)}>×</button><span>ADMIN DETAILS</span><h3>{selected.name || selected.action}</h3><div className="admin-detail-tabs"><b>Overview</b><b>Permissions</b><b>Activity</b><b>Audit History</b></div><dl><dt>Email / Admin ID</dt><dd>{selected.email || selected.actor_email || selected.actor_user_id || selected.user_id || "—"}</dd><dt>Role / Action</dt><dd>{selected.role || selected.action || "—"}</dd><dt>Status / Result</dt><dd>{selected.active ? "Active" : selected.metadata?.result || "Disabled"}</dd><dt>Created / Timestamp</dt><dd>{formatDate(selected.created_at)}</dd><dt>Related record</dt><dd>{selected.target_email || selected.target_name || selected.target_user_id || "—"}</dd><dt>Metadata</dt><dd>{selected.metadata ? JSON.stringify(selected.metadata) : "No additional metadata"}</dd></dl></aside></div>}
      {editTarget && (
